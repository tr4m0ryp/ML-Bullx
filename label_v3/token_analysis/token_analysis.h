...
int token_analysis_structure_filtering(TokenAnalysisData *data){
    ...
    if (creatorRiskLevel) {
        ...
        if (start) {
            sscanf(start, ":%f", &data->creatorRiskLevel);
        }
    }
    ...
    char *amount_topMarketCapCoins = strstr(json_data, "\"amount_topMarketCapCoins\"");
    ...
    //amount_topOgCoins
    char *amount_topOgCoins = strstr(json_data, "\"amount_topOgCoins\"");
    ...
    if (average_marketCap_TMCC) {
        ...
        if (start) {
            sscanf(start, ":%f", &data->average_marketCap_TMCC);
        }
    }
    ...
    if (average_marketCap_TOC) {
        ...
        if (start) {
            sscanf(start, ":%f", &data->average_marketCap_TOC);
        }
    }
