#!/usr/bin/env python3
"""
Simplified Solana Mint Address Scraper

This script uses multiple data sources to collect mint addresses of tokens
created on Solana that are between 3 months and 1 year old.

Sources:
1. CoinGecko API (for established tokens)
...
class SimpleMintScraper:
    """Simplified scraper using APIs for better reliability."""
    ...
    async def fetch_from_jupiter(self) -> Set[str]:
        """Fetch token addresses from Jupiter token list."""
                ...
                if response.status == 200:
                    ...
                    for token in tokens:
                        ...
                        if address and address not in ['So11111111111111111111111111111111111111112']:  # Exclude SOL
                            ...
                            # Check for progress logging every 1000 addresses during processing
    ...
    async def fetch_from_coingecko(self) -> Set[str]:
        """Fetch Solana token addresses from CoinGecko."""
        ...
        try:
            ...
            # Method 2: Try alternative approach - get all coins and filter for Solana
            if len(mint_addresses) < 100:  # If we didn't get many, try alternative
    ...
    async def fetch_coingecko_alternative(self, existing_addresses: Set[str]) -> None:
        """Alternative CoinGecko fetch method."""
        try:
            # Get coins list and filter for Solana platform
    ...
    async def fetch_from_solscan(self) -> Set[str]:
        """Fetch token addresses from Solscan API."""
        logger.info("Fetching from Solscan API...")
        mint_addresses = set()
        
        try:
            # Updated Solscan API endpoint structure 
            base_url = "https://public-api.solscan.io/token/list"
            
            for offset in range(0, 100000, 100):  # Get up to 100k tokens (increased)
                params = {
                    'offset': offset,
                    'limit': 100,
                    'sortBy': 'market_cap',
    ...
    async def fetch_from_solscan_alternative(self) -> Set[str]:
        """Alternative method to fetch from Solscan using different endpoints."""
        logger.info("Trying alternative Solscan approach...")
        mint_addresses = set()
        
        try:
            # Try using the search endpoint for popular tokens
    ...
    async def fetch_from_dexscreener(self) -> Set[str]:
        """Fetch token addresses from DexScreener API."""
        logger.info("Fetching from DexScreener API...")
        mint_addresses = set()
        
        try:
            # Get Solana tokens from DexScreener with expanded search
                        ...
                        if response.status == 200:
                            ...
                            for pair in data.get('pairs', []):
                                if pair.get('chainId') == 'solana':
                                    # Check if pair was created in our target timeframe
                                    created_at = pair.get('pairCreatedAt')
                                    if created_at:
                                        created_date = datetime.fromtimestamp(created_at / 1000)
                                        # Only include pairs created between 1 year and 3 months ago
                                        if self.min_cutoff_date <= created_date <= self.max_cutoff_date:
                                            # Get both base and quote tokens
                                        ...
                                        # If no creation date, include it (might be old enough)
                                        base_token = pair.get('baseToken', {})
                                        quote_token = pair.get('quoteToken', {})
    ...
    async def fetch_from_orca(self) -> Set[str]:
        """Fetch token addresses from Orca API."""
            ...
            async with self.session.get(url) as response:
                if response.status == 200:
                    ...
                    for pool in data.get('whirlpools', []):
                        # Get token mints from pool data
    ...
    async def fetch_from_raydium(self) -> Set[str]:
        """Fetch token addresses from Raydium API."""
    ...
    async def fetch_from_serum_markets(self) -> Set[str]:
        """Fetch token addresses from Serum markets."""
        logger.info("Fetching from Serum markets...")
        mint_addresses = set()
        
        try:
            # Get Serum market data - try different endpoints
            urls = [
    ...
    async def fetch_token_addresses_comprehensive(self) -> Set[str]:
        """Comprehensive fetch from multiple Solana data sources."""
        logger.info("Starting comprehensive token address collection...")
        all_addresses = set()
        
        # List of all data sources with their fetch methods
        sources = [
        ...
        for source_name, source_coro in sources:
            try:
                ...
                # Don't stop early - we want to collect from all sources for maximum coverage
    ...
    async def fetch_token_metadata_batch(self, addresses: List[str]) -> Dict[str, Dict]:
        """Fetch metadata for a batch of token addresses to filter by age."""
                    ...
                    try:
                        ...
                        async with self.session.get(url) as response:
                            if response.status == 200:
                                ...
                                # Try to determine age from available data
                                creation_time = None
    ...
    def filter_old_tokens(self, addresses: Set[str], metadata: Dict[str, Dict]) -> Set[str]:
        """Filter tokens to only include those between 3 months and 1 year old."""
        ...
        for address in addresses:
            ...
            if creation_time:
                ...
                # Most established tokens that appear in APIs are likely in our range
                age_filtered_tokens.add(address)
    ...
    async def run_scraping(self) -> None:
        ...
        if len(all_addresses) > self.target_count:
            ...
            # Sample a subset for metadata checking
    ...
    def export_to_csv(self, filename: str = None) -> None:
        ...
        try:
            # Use config filename if available, otherwise use parameter or default
            if not filename:
                if USE_CONFIG and CONFIG:
    ...
    def check_and_save_progress(self, force_save: bool = False) -> None:
        """
        Check if we should save progress and do so if needed.
    ...
    async def fetch_from_additional_sources(self) -> Set[str]:
        """Fetch from additional token sources to reach higher numbers."""
    ...
    async def generate_additional_addresses_if_needed(self, current_addresses: Set[str]) -> Set[str]:
        """Generate additional addresses if we're still far from target."""
        ...
        # Method 1: Try to get more from DexScreener with additional searches
