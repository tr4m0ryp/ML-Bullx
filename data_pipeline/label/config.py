"""
Token Labeling Configuration and Classification Thresholds.

Centralizes all tunable parameters for the token classification algorithm:
- Rugpull detection criteria (liquidity drops, developer dumps, contract flags).
- Success criteria (price appreciation, volume, holder growth, market cap).
- Unsuccessful token criteria (stagnation, low volume, low adoption).
- API endpoint configuration and rate limiting.
- Data source mappings for each metric category.
- Feature weights and confidence thresholds for the classifier.
