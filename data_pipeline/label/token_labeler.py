...
from shared.constants import ONE_HOUR, THREE_DAYS_SEC, SUSTAIN_DAYS_SEC
from data_pipeline.label.rugpull_vs_success_detector import (
...
from on_chain_solana_pipeline.onchain_provider import OnChainDataProvider
...
class EnhancedTokenLabeler:
                ...
                for token_idx, mint in enumerate(chunk, 1):
                    try:
                        ...
                        if result is not None:
                            ...
                            total_processed += 1
                            logger.info(f"[OK] {mint} -> {result[1]} (progress: {total_processed}/{len(mints)})")
                        else:
                            logger.info(f"[WARN] {mint} -> skipped (no data)")
                    except Exception as e:
                        failed_count += 1
                        logger.error(f"[FAIL] {mint} -> error: {e}")
    ...
    def _log_classification_reasoning(self, m: TokenMetrics, label: str):
        ...
        if label == "inactive":
            ...
            reasons = []
            reasons.append("UNSUCCESSFUL (does not meet success criteria)")
            ...
            reasons.append(f"   ├─ 72h performance: {'PASS' if has_72h_perf else 'FAIL'}")
            if gain_72h:
                reasons.append(f"   │   └─ Actual gain: {gain_72h:.1f}x")
            
            # Sustained performance check
            has_sustained = self._has_sustained_performance(m)
            reasons.append(f"   ├─ Sustained performance: {'PASS' if has_sustained else 'FAIL'}")
            if m.avg_price_post_72h and m.launch_price:
                sustained_ratio = m.avg_price_post_72h / m.launch_price
            ...
            reasons.append(f"   ├─ Community adoption: {'PASS' if has_community else 'FAIL'}")
            ...
            reasons.append(f"   └─ Total appreciation: {'PASS' if (m.mega_appreciation and m.mega_appreciation >= 1000) else 'FAIL'}")
            if m.mega_appreciation:
...
try:
    from data_pipeline.label.enhanced_data_collection import monkey_patch_data_provider
