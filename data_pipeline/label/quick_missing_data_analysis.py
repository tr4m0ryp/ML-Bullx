"""
Quick Missing Data Pattern Analysis from Debug Output.

Provides a fast, offline summary of missing-data patterns observed
during previous debug runs without requiring live RPC access:
- Hard-coded observations from recent debug output.
- Critical missing-data identification with root causes.
- Parsing success-rate statistics.
- Prioritized implementation recommendations split into phases.
...
# =============================================================================
# Offline Debug Analysis
# =============================================================================

def analyze_debug_output():
    """Summarize missing-data patterns from previously collected debug output.

    Uses hard-coded observations from the most recent debug session
    to produce a structured report.  No live data provider calls are
    required.

    Returns:
        Dictionary with the following keys:
            ``critical_missing`` -- list of field names that are most
            frequently absent.
            ``avg_parsing_success`` -- average swap parsing success
            rate as a percentage.
            ``immediate_opportunities`` -- list of fields that can be
            fixed with simple calculations.
    """
    ...
    print("      ROOT CAUSE: Missing total_supply x current_price calculation")
